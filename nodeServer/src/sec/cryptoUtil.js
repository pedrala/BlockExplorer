...
const decKey = async (keyPath, keySeed) => {
    let dec;
...
const readPubkeyPem = async (path, seed) => {
    let decPubKey = await decKey(path, seed);
...
module.exports.getPubkey = async (pubkeyPath) => {
    ...
    //
    let pemRead = await readPubkeyPem(pubkey_path);
